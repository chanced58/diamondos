...
function StatsVisibleSection({ team }: { team: OpponentTeam }): JSX.Element {
  const [error, action] = useFormState(toggleStatsVisibleAction, null);

  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
      <div className="px-5 py-3 bg-gray-50 border-b border-gray-200">
        <h2 className="text-sm font-semibold text-gray-700">Statistics Visibility</h2>
      </div>
      <div className="px-5 py-5">
        {error && (
          <div className="mb-4 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700">
        ...
        <p className="text-sm text-gray-600 mb-3">
          When enabled, full statistics for this team will be visible beyond the opponent context.
          Useful if the team later joins the platform.
        </p>
        <form action={action}>
          <input type="hidden" name="opponentTeamId" value={team.id} />
          <input type="hidden" name="statsVisible" value={team.statsVisible ? 'false' : 'true'} />
          <button
            type="submit"
          ...
          >
            {team.statsVisible ? 'Stats visible — click to hide' : 'Stats hidden — click to show'}
          </button>
        </form>
      </div>
    </div>
...
export function OpponentTeamRosterClient({
  team,
  players,
}: {
  team: OpponentTeam;
