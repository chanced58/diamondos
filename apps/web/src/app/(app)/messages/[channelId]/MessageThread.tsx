...
export function MessageThread({
  ...
  // Group messages: show date separator when the date changes
      ...
      {canPost ? (
        <div className="shrink-0 border-t border-gray-200 bg-white px-6 py-4">
          ...
          <div className="flex items-end gap-3">
            <textarea
              ...
              rows={1}
              className="flex-1 border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
              style={{ minHeight: '42px', maxHeight: '160px', overflowY: 'auto' }}
            />
            <button
              onClick={handleSend}
              disabled={sending || !draft.trim()}
              className="shrink-0 bg-brand-700 text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-brand-800 disabled:opacity-50 transition-colors"
            >
              {sending ? '…' : 'Send'}
            </button>
          </div>
        ...
        <div className="shrink-0 border-t border-gray-200 bg-gray-50 px-6 py-3 text-center">
          <p className="text-xs text-gray-400">
            This channel is read-only for your role. Only coaches can post here.
          </p>
        </div>
      )}
    </div>
