...
export default async function NewEventPage() {
  ...
  if (!isCoach) {
    return (
      <div className="p-8">
        <p className="text-gray-500">Only coaches can create team events.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-lg">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Team Event</h1>
        <p className="text-gray-500 text-sm">{activeTeam.name}</p>
      </div>
      <AddEventForm teamId={activeTeam.id} />
    </div>
  );
}
